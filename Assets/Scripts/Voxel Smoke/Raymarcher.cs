...
public class Raymarcher : MonoBehaviour {
    ...
    private void Awake()
    {
        globalRenderData.raymarcher = this;
    }
    ...
    void Update() {
        ...
        if (smokeVoxelData != null && smokeVoxelData.GetSmokeVoxelBuffer() != null) {
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination) {
        ...
        Debug.Log(smokeVoxelData.GetSmokeOrigin());
    ...
    public void RenderSmoke(CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier destination)
    {
        RenderTexture smokeTex = GetSmokeAlbedoTex();
        RenderTexture smokeMaskTex = GetSmokeMaskTex();

        //Create depth tex for compute shader
        cmd.Blit(source, depthTex, compositeMaterial, 0);
        ...
        if (resolutionScale == Res.HalfResolution)
        {
            cmd.Blit(smokeMaskHalfTex, smokeMaskFullTex);
            cmd.Blit(smokeMaskFullTex, smokeMaskHalfTex);

            if (bicubicUpscale)
            {
                cmd.Blit(smokeAlbedoHalfTex, smokeAlbedoFullTex, compositeMaterial, 1);
            }
            else
            {
                cmd.Blit(smokeAlbedoHalfTex, smokeAlbedoFullTex);
            }
        }

        if (resolutionScale == Res.QuarterResolution)
        {
            cmd.Blit(smokeMaskQuarterTex, smokeMaskHalfTex);
            cmd.Blit(smokeMaskHalfTex, smokeMaskFullTex);
            cmd.Blit(smokeMaskFullTex, smokeMaskHalfTex);
            cmd.Blit(smokeMaskHalfTex, smokeMaskQuarterTex);

            if (bicubicUpscale)
            {
                cmd.Blit(smokeAlbedoQuarterTex, smokeAlbedoHalfTex, compositeMaterial, 1);
                cmd.Blit(smokeAlbedoHalfTex, smokeAlbedoFullTex, compositeMaterial, 1);
            }
            else
            {
                cmd.Blit(smokeAlbedoQuarterTex, smokeAlbedoHalfTex);
                cmd.Blit(smokeAlbedoHalfTex, smokeAlbedoFullTex);
            }
        }
        ...
        cmd.Blit(source, destination, compositeMaterial, 2);
        cmd.Blit(destination, source, compositeMaterial, 2);
    }
}
