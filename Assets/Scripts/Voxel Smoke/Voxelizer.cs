...
public class Voxelizer : MonoBehaviour {
    ...
    private ComputeBuffer staticVoxelsBuffer, smokeVoxelsBuffer, smokePingVoxelsBuffer, argsBuffer, smokeHitVoxelBuff;
    ...
    void OnEnable() {
        ...
        smokeHitVoxelBuff = new ComputeBuffer(2*sizeof(int), sizeof(int));
        int[] test = new int[2];
        test[0] = 0;
        test[1] = 0;
        smokeHitVoxelBuff.SetData(test);
    }
    ...
    void Update() {
        ...
        //if (inputManager.OnFoot.Smoke.triggered)
        //{
        ...
        Debug.Log(staticVoxelsBuffer.count);
    }
    ...
    public bool IsSmokeVoxelHit(Transform position)
    {
        int kernelHandle = voxelizeCompute.FindKernel("CS_HitSmokeVoxel");

        voxelizeCompute.SetVector("_RayHitOrigin", position.position);
        voxelizeCompute.SetBuffer(kernelHandle, "_SmokeVoxels", smokeVoxelsBuffer);
        voxelizeCompute.SetBuffer(kernelHandle, "_IsSmokeVoxel", smokeHitVoxelBuff);
        voxelizeCompute.Dispatch(kernelHandle, 1, 1, 1);

        int[] test = new int[2];

        smokeHitVoxelBuff.GetData(test);

        if (test[0] == 1) { return true; }
