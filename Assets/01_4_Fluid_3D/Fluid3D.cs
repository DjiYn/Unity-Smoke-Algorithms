...
public class Fluid3D : MonoBehaviour 
{
	...
	private int kernel_Temperature = 0;
	...
	void Start () 
	{
        ...
        tempTex = CreateTexture(GraphicsFormat.R16_SFloat); //float divergence
		...
		*/
		...
		shader.SetTexture (kernel_Init, "TempTex", tempTex);
        ...
        shader.SetTexture(kernel_Advection, "TempTex", tempTex);
        ...
        kernel_Temperature = shader.FindKernel("Kernel_Temperature");
        shader.SetTexture(kernel_Temperature, "TempTex", tempTex);
        shader.SetTexture(kernel_Temperature, "VelocityTex", velocityTex);
        shader.SetTexture(kernel_Temperature, "DensityTex", densityTex);
	...
	void FixedUpdate()
	{
		...
		Vector3 npos = new Vector3(0.0f, 0.1f, 0);
		shader.SetVector("spherePos",npos);

		//Send sphere (mouse) velocity
		Vector3 velocity = npos - Vector3.zero;
		...
		shader.SetVector("dyeColor", Color.white);

		//Run compute shader
		DispatchCompute (kernel_Diffusion);
		DispatchCompute(kernel_Temperature);
